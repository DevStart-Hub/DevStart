...
Perfect! We've got our list to which we can append the incoming data. Hoever, just to not have problems using the functions in and out of the function we need a little trick!

We use the `global` keyword inside our callback function. This tells Python that we're referring to the list we created in the main script, not creating a new variable:
