...
Perfect! We've got our list. But there's a catch with Python functions - variables created outside a function aren't automatically accessible inside that function. We need a little trick!

We can use the `global` keyword inside our callback function. This tells Python that we're referring to the list we created in the main script, not creating a new variable:
...
And there we go! Now instead of just printing data, we're actually saving every sample in our list for later analysis. Much more useful!
