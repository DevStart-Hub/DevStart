...
Targets = Raw_data.loc[(Raw_data['Event'] == 'Complex') | (Raw_data['Event'] == 'Simple'), ['time', 'Event']].values
```
...
In our paradigm, the Simple stimulus was always presented on the left (the first AOI we defined) and the Complex stimulus was always presented on the right (the second AOI we defined). Thus we can add this information to our dataframe and then use it to filter out the wrong fixations:

```{python}
#| warning: false
#| message: false
# We define which is the correct AOI that the participant should have looked at.
Target_fixations.loc[Target_fixations['Event'] == 'Simple', 'Correct_Aoi'] = 0
Target_fixations.loc[Target_fixations['Event'] == 'Complex'  , 'Correct_Aoi'] = 1
...
Again, we can now visualize the data. For example, we can plot whether looking time to complex vs simple stimulus:
