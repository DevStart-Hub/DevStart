...
In this tutorial, we'll tackle those variables that just WON'T behave normally! Our focus will be on **Reaction Time**. Remember the initial hypotheses for our study? (if not, you can find an in-depth description of the experimental paradigm in our [intro tutorial](Intro_eyetracking.qmd)) They were not only about Looking Time, but also saccadic reaction times! These reaction times indicate how quickly participants direct their gaze to the target stimuli when they appeared on screen.
...
df$StandardTrialN = standardize(df$TrialN) # standardize the continuous variable
```
...
#| fig-height: 5.5
#| fig-width: 8
ggplot(df, aes(x = Event, y = SaccadicRT)) +
...
Now that we know that our data is not normally distributed and we should try a Gamma distribution, we can update our mixed effect model, making it generalised!

We just need to tweak a couple things. First, instead of using `lmer()` we switch to `glmer()`. Then, we directly specify a distribution which is different from the default Gaussian:
