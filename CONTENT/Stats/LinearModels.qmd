...
What can we spot here? First, our **Intercept** is significant (don't worry, we'll decode this mysterious value later!). Even more exciting, we've got a significant effect of **TrialN** – our continuous variable – (p = `r formatC(parameters::parameters(mod_lm)[2, 9], format = "f", digits = 4)`) and a significant effect of **Event** (p = `r formatC(parameters::parameters(mod_lm)[3, 9], format = "f", digits = 5)`). Double win! However, the interaction between TrialN and Event is playing hard to get with that p = `r formatC(parameters::parameters(mod_lm)[4, 9], format = "f", digits = 3)`. That's nowhere near significant!
...
ggplot() +
  ...
  annotate("text", x = 0.20, y = 1260, label = expression(beta), size = 4) +
...
ggplot() +
  ...
  geom_pointrange(aes(x=-.15, y=intercept_Simple, ymin= intercept_Simple-intercept_Simple_se, ymax= intercept_brush+intercept_Simple_se, color = 'Simple'), lwd=1.3)+
  geom_pointrange(aes(x=.15, y=intercept_Complex, ymin= intercept_Complex-intercept_Complex_se, ymax= intercept_Complex+intercept_Complex_se, color = 'Complex'), lwd=1.3)+
  
  geom_segment(aes(x = 0, xend = 0, y = intercept_Simple, yend = intercept_Complex), 
               color = "black", lwd = 1.2,
               arrow = arrow( length = unit(0.1, "inches"), ends = "both")) +
  annotate("text", x = -0.05, y = intercept_Simple - (intercept_Simple-intercept_Complex)/2, label = expression(beta), size = 4) +
  
  # Customize the color scale for the legend
  scale_color_manual( values = color_map) +
  coord_cartesian(xlim = c(-1,3), ylim = c(1000,1400))+
...
ggplot() +
  ...
  geom_pointrange(aes(x=0, y=intercept_brush, ymin= intercept_brush-intercept_brush_se, ymax= intercept_brush+intercept_brush_se, color = 'Simple'), lwd=1.3)+
  
  annotate("text", x = 4.4, y = 1162, label = expression(beta), size = 4) +
  geom_path(aes(x = c(4, 4, 10), y = c(1250, 1150, 1150)), 
  ...
  coord_cartesian(xlim = c(-1,20), ylim = c(1000,1400))+
...
-   For **Simple** events, **LookingTime** decreases by 7.5 units per trial (that's our main `TrialN` coefficient)

-   The interaction coefficient (0.13) tells us how this slope changes for **Complex** events
...
ggplot()+
  ...
  geom_pointrange(aes(x =-.15, y = intercept_brush, ymin = intercept_brush - intercept_brush_se,ymax = intercept_brush + intercept_brush_se, color = 'Simple' ),
                  lwd  = 1.3) +
  geom_pointrange(aes(x=.15, y=intercept_Complex, ymin= intercept_Complex-intercept_Complex_se, ymax= intercept_Complex+intercept_Complex_se, color = 'Complex'),
                  lwd  = 1.3)+
