...
In this tutorial, we will use a linear model to test one of our initial hypotheses, so we suggest to go check them out [here](/CONTENT/EyeTracking/Intro_eyetracking.qmd) if you haven't done so already. We want to test whether infants look longer at a Complex stimulus compared to a Simple stimulus. Given that infants see repeated presentations of the same stimuli, it might also be worth checking whether this effect changes over time! It might either get stronger or disappear...
...
We’ll be discussing linear models in a very accessible and practical manner. Even if explanations might not align with the rigorous definitions statisticians are accustomed to, we provide a stepping stone for you to grasp the concept of linear models and similar analyses. Let’s get started!
:::
...
Linear models offer a perfect balance: they're accessible enough for beginners to grasp, yet powerful enough to satisfy the needs of advanced researchers. As we dive deeper into this topic, you'll see just how these seemingly simple tools can unlock complex insights in your data.
...
In the tutorials regarding stats, we will use R (and Positron as the IDE). The concepts and procedures are the same in other programming languages (e.g., Python, Julia). However, we choose R for a simple reason. R is just better at most statistical things!!
:::
...
long_data <- tibble(
  ...
  Event = rep(rep(c("Complex", "Simple"), each = 3), 3),
  TrialN = rep(1:3, 6),
  LookingTime = c(
    # P01 - Complex condition
    1450, 1420, 1390,
    # P01 - Simple condition
    1380, 1350, 1320,
    # P02 - Complex condition
    1480, 1460, 1410,
    # P02 - Simple condition
    1390, 1370, 1340,
    # P03 - Complex condition
    1520, 1490, 1450,
    # P03 - Simple condition
    1410, 1380, 1350
  )
)
...
What can we spot here? First, our **Intercept** is significant (don't worry, we'll decode this mysterious value later!). Even more exciting, we've got a significant effect of **TrialN** (p = `r formatC(parameters::parameters(mod_lm)[2, 9], format = "f", digits = 4)`) and of **Event** (p = `r formatC(parameters::parameters(mod_lm)[3, 9], format = "f", digits = 5)`). However the interaction between **TrialN** and **Event** (p = `r formatC(parameters::parameters(mod_lm)[4, 9], format = "f", digits = 3)`) does not seem to be significant.
...
-   **Residuals**: This section shows the leftovers of our model - the differences between what we observed and what our model predicted. Think of residuals as the bits that our model couldn't explain. We want these to be as small as possible! 🔍
...
2.  We're in the **Simple Stimulus** condition
...
color_map <- c("Simple" = "darkred", "Complex" = 'darkblue')
...
ggplot() +
  ...
  annotate("text", x = 0.3, y = 1260, label = 'beta', parse= T,size = 8) +
...
It sounds a bit confusing, but let's break it down. If we want to understand what is the estimate for a **Complex** event we need to take the **Intercept** (`r formatC(parameters::parameters(mod_lm)[1, 2], format = "f", digits = 2)`) –as we mentioned that is actually the **Complex** event – and then just simply add to it the Estimate for the `Event [Simple]` (`r formatC(parameters::parameters(mod_lm)[3, 2], format = "f", digits = 2)`). So the model is telling us that a **Complex** event should be `r formatC(parameters::parameters(mod_lm)[1, 2], format = "f", digits = 2)`- `r formatC(parameters::parameters(mod_lm)[3, 2], format = "f", digits = 2)`= `r formatC(parameters::parameters(mod_lm)[3, 2] + parameters::parameters(mod_lm)[3, 2], format = "f", digits = 2)`.

See? Not too bad! Let's visualize it and make it even clearer!

```{r}
#| label: plot mean diff
#| echo: false
#| message: false
#| warning: false
#| fig-dpi: 300
#| fig-height: 8
#| fig-width: 10
# Define intercept and slope for the Simple
intercept_Simple <- model_p[1,2]
intercept_Simple_se <- model_p[1,3]

intercept_Complex <- model_p[3,2] + intercept_Simple
intercept_Complex_se <- model_p[3,3]
...
ggplot() +
  ...
  geom_pointrange(aes(x=-.15, y=intercept_Simple, ymin= intercept_Simple-intercept_Simple_se, ymax= intercept_brush+intercept_Simple_se, color = 'Complex'), lwd=1.3)+
  geom_pointrange(aes(x=.15, y=intercept_Complex, ymin= intercept_Complex-intercept_Complex_se, ymax= intercept_Complex+intercept_Complex_se, color = 'Simple'), lwd=1.3)+
  
  geom_segment(aes(x = 0, xend = 0, y = intercept_Simple, yend = intercept_Complex), 
               color = "black", lwd = 1.2,
               arrow = arrow( length = unit(0.1, "inches"), ends = "both")) +
  annotate("text", x = +0.07, y = intercept_Simple - (intercept_Simple-intercept_Complex)/2, label = 'beta',parse = T, size = 4) +
  
  # Customize the color scale for the legend
  scale_color_manual( values = color_map) +
  coord_cartesian(xlim = c(-1,2), ylim = c(1250,1600))+
...
# Create a small data frame for the main (Simple) line
df_p <- data.frame(
  time  = time_values,
  est_performance_EventTrial = intercept_Simple + slope_EventTrial * time_values
)


color_map <- c("Simple" = "darkred", "Complex" = 'darkblue', "EventTrial" = '#e04d01')


ggplot() +
  geom_vline(xintercept = 0, lwd =0.4,linetype = "dashed") +

  geom_line(data = df_p, aes(x = time, y = est_performance_EventTrial), color = 'darkblue', size  = 1) +
  # geom_pointrange(aes(x  =0, y = intercept_brush, ymin = intercept_brush - intercept_brush_se,ymax = intercept_brush + intercept_brush_se, fill = 'Simple' ),
  #                 color = 'black', shape = 21, 
  #                 lwd = 1.3,fatten = 8) +
  
  
    
  geom_pointrange(aes(x=0, y=intercept_brush, ymin= intercept_brush-intercept_brush_se, ymax= intercept_brush+intercept_brush_se, color = 'Complex'), lwd=1.3)+
  
  annotate("text", x = 4.4, y = 1400, label = 'beta', parse= T, size = 4) +
  geom_path(aes(x = c(4, 4, 12), y = c(1490, 1390, 1390)), 
  ...
  coord_cartesian(xlim = c(-1,20), ylim = c(900,1600))+
...
-   For **Complex** events, **LookingTime** decreases by `r formatC(parameters::parameters(mod_lm)[2, 2], format = "f", digits = 2)` units per trial (that's our main `TrialN` coefficient)

-   The interaction coefficient (`r formatC(parameters::parameters(mod_lm)[4, 2], format = "f", digits = 2)`) tells us how this slope changes for **Simple** events

So for **Simple** events, the slope would be: `r formatC(parameters::parameters(mod_lm)[2, 2], format = "f", digits = 2)`+ `r formatC(parameters::parameters(mod_lm)[4, 2], format = "f", digits = 2)` = `r formatC(parameters::parameters(mod_lm)[2, 2] + parameters::parameters(mod_lm)[4, 2], format = "f", digits = 2)` units per trial.
...
#| fig-height: 8
#| fig-width: 10
# Define intercept and slope for the Simple condition
intercept_Simple <- model_p[1,2]  # Intercept for Simple
intercept_Simple_se <- model_p[1,3]

# Define intercept for Complex condition (Intercept + Complex Effect)
intercept_Complex <- model_p[3,2] + intercept_Simple
intercept_Complex_se <- model_p[3,3]
...
df_p <- data.frame(
  time  = time_values,
  est_performance_EventTrial = intercept_Simple + slope_EventTrial * time_values,
  est_performance_Interaction = intercept_Complex + (slope_EventTrial + slope_Interaction) * time_values
)
...
ggplot()+
  ...
  geom_line(data = df_p, aes(x = time, y = est_performance_EventTrial), color = "darkblue", lwd  = 1) +
  geom_line(data = df_p, aes(x = time, y = est_performance_Interaction), color = "darkred", lwd  = 1)+
  
  geom_pointrange(aes(x =-.15, y = intercept_brush, ymin = intercept_brush - intercept_brush_se,ymax = intercept_brush + intercept_brush_se, color = 'Complex' ),
                  lwd  = 1.3) +
  geom_pointrange(aes(x=.15, y=intercept_Complex, ymin= intercept_Complex-intercept_Complex_se, ymax= intercept_Complex+intercept_Complex_se, color = 'Simple'),
                  lwd  = 1.3)+
  ...
  annotate("text", x = 6.4, y = 1150, label = 'beta', parse= T, size = 4) +
  geom_path(aes(x = c(6, 6, 10.5), y = c(1240, 1140, 1140)), 
          color = "black", lwd = 1.2, arrow = arrow( length = unit(0.1, "inches"), ends = "both"))+
...
model_p_std = parameters(mod_lm_standardized)

# Define intercept and slope for the NoReward
intercept_brush <- model_p_std[1,2]
intercept_brush_se <- model_p_std[1,3]

# Define intercept and slope for the Simple condition
intercept_Simple <- model_p_std[1,2]  # Intercept for Simple
intercept_Simple_se <- model_p_std[1,3]

# Define intercept for Complex condition (Intercept + Complex Effect)
intercept_Complex <- model_p_std[3,2] + intercept_Simple
intercept_Complex_se <- model_p_std[3,3]

# Define slopes
slope_EventTrial <- model_p_std[2,2]  # Slope for Event Trial
slope_Interaction <- model_p_std[4,2] # Interaction effect

# Some hypothetical TrialN values
time_values <- seq(min(df$StandardTrialN), max(df$StandardTrialN), by = .05)
# index <- which.min(abs(time_values - (-0.15)))
...
ggplot()+
  ...
  geom_pointrange(aes(x =-.018, y = intercept_brush, ymin = intercept_brush - intercept_brush_se,ymax = intercept_brush + intercept_brush_se, color = 'Complex' ),
                  lwd  = 1.3) +
  geom_pointrange(aes(x=.018, y=intercept_Complex, ymin= intercept_Complex-intercept_Complex_se, ymax= intercept_Complex+intercept_Complex_se, color = 'Simple'),
                  lwd  = 1.3)+  
  
  scale_color_manual( values = color_map) +
  coord_cartesian(xlim = c(-1.7,1.7), ylim = c(900,1600))+
  ...
  theme(legend.position = 'bottom')+
    # Customize x-axis labels
  scale_x_continuous(
    breaks = seq(-2,2,.5),              # The x-values where you want ticks
    labels = ifelse(
    as.character(seq(-2, 2, 0.5)) == "0",
    "0\n(Intercept)",
    as.character(seq(-2, 2, 0.5))
)  # Corresponding custom labels
  )
```
...
We have run our model and we looked at the summary right away... We just couldn't resist! But before doing that, we should've actually checked if our model was actually ok. Linear models, like most statistical techniques require some data assumption to be run. These assumption need to be met otherwise even if our model could be showing amazing results, it won't be valid.
...
Again this is not a theoretical tutorial. So we won't go into details as which are the assumptions (please read some of the link provided though!!) but we will show you how to actually check these assumptions.
...
As easy as that!! We have a plot of the model assumptions and we can check if they are met!! But what do these plots represent? Here below we created a table that includes each plot with a brief explanation. If you want to understand more about the [`check_model()`](https://easystats.github.io/performance/reference/check_model.html) function we suggest you to read the documentation about it and also the [very nice vignette](https://easystats.github.io/performance/articles/check_model.html) that the package provides.
