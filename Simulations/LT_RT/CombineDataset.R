...
SaccadicRT <- vroom::vroom("Simulations/LT_RT/simulatedLognormal.csv") %>% 
  ...
  select(Id, Event, TrialN, SaccadicRT)  # Keep only relevant columns
...
parameters(mod_l, effects = 'fixed')
check_model(mod_l)
plot(check_outliers(mod_l))
plot(check_predictions(mod_l))
plot(check_heteroscedasticity(mod_l))
# Reaction Time Model ---------------------------------------------------------
...
mod_gam <- glmer(SaccadicRT ~ Event * Stand_TrialN + 
                   (1 + Stand_TrialN | Id),
                 family = Gamma(link = 'log'), 
                 data = Df)
check_model(mod_gam)

plot(check_outliers(mod_gam))
plot(check_predictions(mod_gam))
...
D2 <- ggplot(Df, aes(x = SaccadicRT, fill = Event)) +
  geom_density(color = 'transparent', alpha = 0.3) +      # Filled density curves
  geom_density(fill = 'transparent', color = 'black', lwd = 2) +  # Overall outline
  labs(
